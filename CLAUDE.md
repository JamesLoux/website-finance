...
- [x] Simulateur de positions (book jusqu'à 4 trades, modale création/édition, 4 graphiques Greeks interactifs)
...
- `/cours/module-5-produits-equity` — `delta-one-cash` ✅, `options-vanilles` ✅, `strategies-optionnelles` ✅ (module complet, sidebar entièrement cliquable, carte active dans le chemin serpent) — **affiché Module 7 — Equity I**
...
app/
  ...
  simulateur/page.js                 ← Simulateur : book de 4 trades max (slots 0→3, couleur figée par slot), modale unique création/édition, 4 graphiques Greeks, bandeau strikes hétérogènes
...
- **2026-09-03 — Simulateur : 4 trades, édition, héritage de sous-jacent, avertissement d'agrégation** :
  - Trois évolutions de `app/simulateur/page.js`, plus un bandeau d'avertissement. Aucune autre page touchée ; axe X, scaling euros et convention de taille inchangés.
  - **Book porté de 2 à 4 trades.** `MAX_TRADES = 4`, palette `TRADE_COLORS = ['#2563eb', '#dc2626', '#16a34a', '#d97706']` (bleu, rouge, vert, ambre — pas de jaune pur, illisible en trait fin sur blanc).
  - **⚠️ La notion de `slot` est le point central de la session, et il faut la préserver dans toute évolution ultérieure.** Chaque trade porte un `id` (compteur `useRef`, jamais réutilisé) **et** un `slot` entier 0→3 attribué par `firstFreeSlot()` = plus petit slot libre. La couleur (`TRADE_COLORS[slot]`), le numéro affiché (`slot + 1`) et le libellé de courbe (`T{slot+1}: ...`) dérivent tous du slot, **jamais de l'index du tableau**. Fermer un trade libère son slot ; le tableau `trades` est maintenu trié par slot, si bien que le trade de référence (celui qui porte le `r` partagé et le sous-jacent hérité) est toujours `trades[0]`. Vérifié dans le navigateur : fermer le trade 1 laisse 2/3/4 avec leurs couleurs, et le trade suivant ouvert reprend le slot 0 et redevient bleu « Trade 1 ».
  - **`buildDatasets` boucle désormais sur le book** au lieu du cas dur 1/2 trades. 1 trade : courbe pleine à sa couleur. 2 à 4 trades : une pointillée par trade à sa couleur + la noire agrégée (somme simple des Greeks en euros, inchangée). Les zones `_fill_pos`/`_fill_neg` restent indexées sur la courbe de référence (la bleue à 1 trade, la noire dès 2).
  - **Checkbox « Agrégé seul »** (à côté de la modale ⓘ, visible dès 2 trades) : masque les courbes individuelles **et leurs points au spot**, garde les zones colorées et la noire. Les verticales pointillées du plugin restent affichées (une par trade ouvert, à sa couleur, opacité 0,45) et la colonne de valeurs numériques reste complète — la case ne pilote que les courbes.
  - **Édition d'un trade ouvert** : `editingId` (null = création), bouton crayon ✎ à gauche du × sur chaque carte. **Une seule modale sert aux deux modes**, seuls le titre, le libellé du bouton et la présence du toggle de sous-jacent changent. À l'enregistrement, `id` et `slot` sont conservés (donc couleur et numéro), la prime et `N = montant / S` sont recalculés.
  - **Verrou du champ `r` généralisé** : en création il est verrouillé dès qu'un trade existe (inchangé) ; en édition il est verrouillé **sauf sur le trade de référence**. Sans cette exception le `r` du book aurait été définitivement figé après le premier trade. Modifier le `r` de référence le propage aux autres trades **et recalcule leur prime** (vérifié : `r` 4 % → 9 % fait passer la prime du trade 2 de 28,22 € à 31,75 €).
  - **Toggle « Même sous-jacent que le trade N ? »** en tête de modale, **création uniquement** et seulement si le book n'est pas vide. Défaut Oui : préremplit S, σ et q depuis le trade de référence. Non : ces trois champs reviennent aux valeurs par défaut. Ni K, ni T, ni le type, ni le sens, ni le montant ne sont hérités ; `r` n'est pas concerné (verrouillé et hérité dans les deux cas).
  - **⚠️ Sur « Oui », S et q sont verrouillés — σ ne l'est pas.** Deux champs hérités sont grisés et non modifiables (`sousJacentHerite = editingId === null && !!refTrade && sameUnderlying`, porté par `lock` sur ces seuls champs) : un même sous-jacent a un seul spot et un seul rendement de dividende, les laisser libres permettait d'ouvrir deux trades « sur le même sous-jacent » avec des S contradictoires. **σ reste délibérément modifiable** : la volatilité implicite dépend du strike et de la maturité, deux options sur le même sous-jacent ont légitimement des vols différentes — c'est tout l'objet du smile. Repasser sur « Non » libère les trois champs. Le message sous un champ verrouillé distingue les deux verrous : « Partagé avec le trade N pour cohérence du pricing » pour `r`, « Hérité du trade N — répondre « Non » ci-dessus pour le modifier » pour S et q.
  - **Aucun verrou de sous-jacent en édition** : le toggle n'y étant pas affiché, S, σ et q y sont toujours libres. Seul `r` garde sa règle propre (éditable sur le seul trade de référence).
  - **Bandeau d'avertissement sur l'agrégation** (boîte amber, sous la ligne des checkboxes, au-dessus de la grille) dès que le book contient au moins deux strikes K distincts : l'axe étant gradué en moneyness S/K, un même point de l'axe correspond à un spot différent pour chaque trade, donc la courbe noire n'a pas de sens financier dans cette configuration. Purement informatif, aucun calcul modifié.
  - **⚠️ Les graphiques ne sont plus recréés à chaque changement du book** — le `useEffect([trades, visibleGreeks])` unique qui détruisait puis recréait les 4 charts est remplacé par le motif à deux effets déjà en vigueur dans `ConvexityChart` et `ForwardCurveChart` :
    1. **Effet de structure**, dépendance `[structureKey]` où `structureKey = JSON.stringify({ slots, aggOnly, visibleGreeks })` — tout ce qui change le **nombre** ou l'**ordre** des datasets. Lui seul crée et détruit les charts. Il lit `tradesRef.current` (assignée dans le corps du composant, plus dans un effet) pour ne pas dépendre de `trades`.
    2. **Effet de données**, dépendance `[trades, structureKey]` : réassigne `data` et `label` dataset par dataset puis `chart.update('none')`, avec un garde-fou `next.length !== chart.data.datasets.length` qui laisse la main à l'effet de structure. Éditer un trade passe donc **uniquement** par ce second effet. Les couleurs ne sont volontairement pas remises à jour ici : elles dérivent du slot, qui ne bouge pas tant que le trade est ouvert — et si un slot change, `structureKey` change et le chart est recréé.
  - **Vérification** : harnais Node reprenant le motif d'extraction de `scripts/payoff-tests/` (le moteur est découpé **dans** `app/simulateur/page.js`, jamais recopié) — 53 contrôles verts sur l'allocation de slots, la structure des datasets pour 1 à 4 trades, l'égalité exacte agrégée = somme des individuelles (écart < 1e-12), le mode « Agrégé seul », la numérotation par slot et la coïncidence point au spot / valeur affichée. Puis pilotage de Chrome headless via le protocole DevTools (sans Puppeteer, WebSocket natif de Node) pour rejouer les parcours réels : ouverture de 4 trades, bouton désactivé « Book plein », édition, fermeture/réouverture d'un slot, toggle de sous-jacent, propagation du `r`, « Agrégé seul ». Aucune erreur console.
  - **⚠️ Piège de méthode rencontré, à connaître pour tout futur script de pilotage navigateur** : un script injecté via une *template literal* JS voit ses `\d` et `\s` mangés par l'interpolation (`\d` → `d`). Une regex `/Contrats([\d.]+)/` devient `/Contrats([d.]+)/` et échoue silencieusement, et un `.replace(/\s+/g,' ')` devient `.replace(/s+/g,' ')` qui mutile le texte (« Contrats » → « Contrat »). Utiliser `[0-9.]` plutôt que `[\d.]` dans ces scripts, ou doubler les échappements.
- **2026-06-10** :
...
- **2026-09-02 (suite 3) — Activation du Module 7 dans le chemin serpent** :
  ...
  - **Première rangée mixte du site.** Les rangées 1→3, 4→6 et 10→12 étaient uniformément actives, la rangée 7→9 uniformément inactive. Activer le Module 7 seul crée le premier cas où une carte à bordure 2px `#1d4ed8` voisine deux cartes à bordure 1,5px `#bfdbfe`. Vérification faite au pixel près (capture Chrome headless à `--force-device-scale-factor=3`, PNG décodé en Python pur) :
    - **Bords extérieurs parfaitement affleurants** : les trois cartes ont exactement le même y de bord haut (2541 en px device 3×) et de bord bas (2910). `box-sizing: border-box` (preflight Tailwind v4) fait que `width: 208px` inclut la bordure : la largeur extérieure est identique quelle que soit l'épaisseur, donc **aucun décalage horizontal, la rangée reste à 752px et les connecteurs restent alignés**.
    - **Connecteurs `HConn` insensibles au cas mixte** : ils sont positionnés en `alignSelf: 'flex-start'; marginTop: 12` **par rapport au haut de la rangée**, pas par rapport aux cartes voisines, et dessinés dans leur propre SVG (ce ne sont pas des traits tendus entre deux ancres). Un connecteur ne peut donc pas se retrouver « penché » entre une carte fine et une carte épaisse. Mesuré : les flèches 7→8 et 8→9 occupent exactement la même bande (y 2598–2615, 18 px device = 6 px CSS de tige). Leur couleur est `#9ca3af` en dur, indépendante de l'état des cartes qu'elles relient — pas de rupture de teinte à la jonction actif/inactif.
    - **Hauteur de rangée** : `alignItems: 'stretch'` sur `Row` + `flex: 1` sur la partie basse de `Node` — le passage des sous-pages de `<span>` à `<a>` ne change pas la hauteur (mêmes 12px/1.4), la rangée ne bouge pas.
  - **⚠️ Seul artefact réel, mesuré et laissé en l'état : un décalage vertical de 1px CSS du contenu interne.** La bordure étant à l'intérieur de la boîte, le contenu de la carte 7 est poussé de 0,5px CSS de plus que celui des cartes 8 et 9. Mesuré à 3× : haut du glyphe du numéro à y=2586 pour le « 7 » contre 2583 pour le « 8 » et le « 9 » (3 px device = 1 px CSS), filet séparateur à 2673 contre 2671 (0,67 px CSS). Invisible à l'œil au zoom normal, mais c'est un vrai décalage et il ne peut apparaître que sur une rangée mixte. **Correctif d'une ligne si le propriétaire le souhaite** : passer les deux états à `2px` dans `Node` et ne différencier que par la couleur (`#1d4ed8` vs `#bfdbfe`) — la hiérarchie visuelle actif/inactif est déjà portée par la couleur du trait, du numéro, du titre et du fond bas. Non appliqué : c'est un choix de design, pas un bug.
  - **`app/quiz/page.js` volontairement non modifié** : `isAvailable` reste `["01","02","03","04","05","06","10","11","12"]`, le Module 7 garde son badge « Bientôt disponible » puisque le quiz n'existe pas encore.
  - **Aucun lien vers un index de module** : le composant `Node` ne rend jamais le titre cliquable, seules les sous-pages portent un `href`. Vérifié sur le HTML rendu : aucune occurrence de `href="/cours/module-..."` sans segment de sous-page, et il n'existe effectivement pas de `page.js` à la racine de `module-5-produits-equity/`. Les 3 sous-pages répondent 200.
...
- **2026-05-15** :
  - **`app/cours/page.js` — améliorations visuelles du chemin serpent** :
    - **Correction des traits verticaux coupés** : `Row` passe de `alignItems: 'flex-start'` à `alignItems: 'stretch'` — toutes les cartes d'une rangée atteignent la même hauteur. `Node` passe en `display: flex; flexDirection: column` avec `flex: 1` sur la partie basse — la carte s'étire jusqu'au bas de la rangée. Les connecteurs verticaux (`VConn`) partent désormais du bas des cartes courtes (Module 3, 6, 9) sans espace flottant.
    - **Flèches directionnelles sur les connecteurs** : `HConn` et `VConn` réécrits en SVG. `HConn` affiche une tige + triangle selon la direction : `→` sur les rangées gauche→droite (1→3, 7→9), `←` sur la rangée inversée (4→6) via prop `reverse`. `VConn` affiche une tige + triangle `↓` à l'extrémité basse. Épaisseur de tige (`sw`) = 6px. Taille de la pointe : `ARROW = 8`, perpendiculaire doublée (`±ARROW` au lieu de `±ARROW/2`) pour des flèches larges. `HConn` invisible conservé pour l'alignement de la rangée 10.
    - **Bordure des modules actifs** : couleur `#1d4ed8` (bleu-700, plus saturé que l'ancien `#2563eb`) et épaisseur 2px (contre 1.5px pour les modules inactifs). ⚠️ Le code dit bien `2px`, pas 3px comme indiqué dans une version antérieure de cette note.
    - **Fond papier millimétré** : div `position: fixed; inset: 0; zIndex: -1` avec `backgroundColor: '#ffffff'` et grille CSS (`repeating-linear-gradient`) — grandes lignes tous les 100px (`rgba(37,99,235,0.10)`) et petites lignes tous les 20px (`rgba(37,99,235,0.045)`). Aucun canvas, aucun `useEffect`. Encapsulé dans un fragment `<>…</>`.
  - **`app/quiz/page.js` — refonte complète de la mise en page** :
    - **Fond papier millimétré** : même div `position: fixed; zIndex: -1` que la page cours. `bg-gray-50` supprimé du wrapper principal.
    - **Layout 1 quiz par ligne** : remplacement de la grille `grid-cols-2` par une liste `flex flex-col gap-2`. Chaque carte passe de `p-6 flex-col` à `h-[50px] flex items-center gap-4 overflow-hidden` — format horizontal compact avec hauteur fixe identique pour tous les modules. Colonnes : numéro (`w-7`, bleu pâle) · titre (`w-40`, semibold) · sous-pages (`flex-1`, `line-clamp-2`, masqué sur mobile) · questions (`w-24`, masqué sur mobile) · badge. `line-clamp-2` remplace `truncate` pour afficher les sous-pages sur 2 lignes sans dépasser la hauteur fixe. `max-w-5xl` → `max-w-3xl`, `py-16` → `py-12`, header `text-4xl mb-12` → `text-3xl mb-8`.
